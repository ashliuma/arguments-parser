...
TEST(ArgParserTestSuite, HelpStringTest) {
    ...
    ASSERT_TRUE(parser.Parse(SplitString("app --help")));
     ASSERT_EQ(
         ...
         "My Parser\n"
         "-i --input File path for input file [default=false, not positional, multivalue, min args=1]\n"
         "- --numer Some Number [default=false, not positional, one value]\n"
         "-h --help Display this help and exit [default=false, not positional, one value]\n"
     );
}
